...
export default function EventLogistics() {
    ...
    const fetchData = async () => {
      ...
      if (eventData) {
        if (eventData.artist_id) {
          const { data: artistData } = await supabase.from('artists').select('*').eq('id', eventData.artist_id).single();
          setArtist(artistData || null);
        }

        const { data: travelData } = await supabase.from('travel_segments').select('*').eq('event_id', eventData.id);
        setTravel(travelData || []);

        const { data: hotelData } = await supabase.from('accommodations').select('*').eq('event_id', eventData.id).single();
        setHotel(hotelData || null);

        const { data: guestData } = await supabase.from('guestlist').select('*').eq('event_id', eventData.id);
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-6 space-y-6">
      ...
      {/* ✈️ FLIGHT TRACKING WIDGET */}
      <FlightWidget flightNumber="LH401" />
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded shadow">
          ...
          {travel.map((t) => (
            <div key={t.id} className="mb-3">
              <p className="font-medium">FLIGHT: {t.dep_city} → {t.arr_city}</p>
